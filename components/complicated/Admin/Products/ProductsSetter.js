...
export default function ProductsSetter() {
  ...
  // console.log("🚀 ~ file: ProductsSetter.js ~ line 19 ~ ProductsSetter ~ productList", productList)
  const descs = [
  ...
  return (
    <div className={`font-rc`}>
      {update && (
        <div
          className={`fixed top-2 left-2 bg-belplit_2 bg-opacity-90 text-white text-sm font-light rounded-sm px-2 py-0.5 animate-pulse`}
        >
          сохранить
        </div>
      )}
          ...
          return (
            <div key={`asjdifh${iii}`}>
              {cities[selectedCity][1] === item_iii[1] && (
                <Layout update={update} save={save}>
                                                ...
                                                <React.Fragment>
                                                  <div className={`absolute w-full h-6 z-50 flex gap-2 my-1`}>
                                                    <input
                                                      className={`w-full border rounded-sm `}
                                                      value={
                                                        settings[i]?.content?.[dii]?.[descItem[1].length + 1]
                                                      }
                                                      onChange={(e) => {
                                                        setSettings((s) => ({
                                                          ...s,
