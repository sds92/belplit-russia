...
import { Layout, AddProduct, ProductLine } from './components';

import { useSelector, useDispatch } from "react-redux";
import { update, selectProducts } from "redux/slices/productsSlice";
...
export default function ProductsSetter() {
  ...
  return (
    <div className={`font-rc`}>
      {updateStatus && (
        <div
          className={`cursor-pointer fixed bottom-2 left-2 bg-belplit_2 bg-opacity-90 text-white text-sm font-light rounded-sm px-2 py-0.5 animate-pulse`}
        >
          сохранить
        </div>
      )}
      ...
      <div className={``}>
        ...
        >
          ...
          <Layout.Products updateStatus={updateStatus} save={save}>
            <AddProduct addProduct={addProduct} />

            {productsUpd.map((item, i) => {
              return (
                <React.Fragment key={`lfjkh${i}`}>
                  <ProductLine product={item} productList={productList}/>
                  {/* SETTINGS */}
                  {settings[i]?.open ? (
                    <tr>
                      <td colSpan={7}>
                                                ...
                                                content: {
                                                  meta: {
                                                    [metaItem.toLowerCase()]:
                                                      metaItem.toLowerCase() === 'title'
                                                        ? pages.find((item) => item.content.product_id === i)
                                                            ?.head.title
                                                        : pages
                                                            .find((item) => item.content.product_id === i)
                                                            .head.meta.find(
